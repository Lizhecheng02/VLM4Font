...
def run(args):
    ...
    for model_name in models:
        ...
        for folder, images in folder_images.items():
            ...
            for img_path in tqdm(images, total=len(images)):
                if "gpt" in model_name.lower():
                    response = get_response_vlm_openai(model=model_name, prompt=get_zero_shot_CoT_prompt(), image_path=img_path, openai_api_key=openai_api_key)
                    ...
                    response = phi_vlm_inference(model=model, processor=processor, prompt=get_zero_shot_CoT_prompt(), image_path=img_path)
                elif "qwen" in model_name.lower():
                    response = qwen2_vlm_inference(model=model, processor=processor, prompt=get_zero_shot_CoT_prompt(), image_path=img_path)
                elif "llama" in model_name.lower() and "idefics" not in model_name.lower() and "90" not in model_name.lower():
                    ...
                    response = idefics_vlm_inference(model=model, processor=processor, prompt=get_zero_shot_CoT_prompt(), image_path=img_path)
        ...
        print("-" * 60)
        print(f"Zero-Shot CoT | Model: {model_name.split('/')[-1]} | Accuracy: {correct_count / total_count * 100: .2f}%")

        os.makedirs(f"./results/name/{model_name.split('/')[-1]}", exist_ok=True)
        output_filename = f"./results/name/{model_name.split('/')[-1]}/zscot.json"
...
def run_sentence(args):
    ...
    for model_name in models:
        ...
        output_filename = f"./results/sentence/{model_name.split('/')[-1]}/zscot.json"
        with open(output_filename, "w", encoding="utf-8") as f:
            json.dump(results, f, ensure_ascii=False, indent=4)
        print(f"Results Saved to {output_filename}")
